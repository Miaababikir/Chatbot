...
/*
            ...
            if(sPrevInput.equalsIgnoreCase(sInput) && sPrevInput.length() > 0) {
                ...
                Label userRepeat = new Label("YOU ARE REPEATING YOURSELF.");
                vBox.getChildren().add(userRepeat);
            } else if(sInput.equalsIgnoreCase("BYE")) {
                Label botQuit = new Label("IT WAS NICE TALKING TO YOU USER, SEE YOU NEXT TIME!");
                vBox.getChildren().add(botQuit);
            } else if(responses.size() == 0) {
                // handles the case when the program doesn't understand what the user is talking about
                Label botNull = new Label("I'M NOT SURE IF I UNDERSTAND WHAT YOU ARE TALKING ABOUT.");
                vBox.getChildren().add(botNull);
